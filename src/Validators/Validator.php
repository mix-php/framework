...
class Validator extends BaseObject
{
    ...
    public function setScenario($scenario)
    {
        ...
        if (!isset($scenarios[$scenario])) {
            throw new \Mix\Exceptions\InvalidArgumentException("场景不存在：{$scenario}");
        }
        if (!isset($scenarios[$scenario]['required'])) {
            throw new \Mix\Exceptions\InvalidArgumentException("场景 {$scenario} 未定义 required 选项");
        }
    ...
    public function validate()
    {
        if (!isset($this->_scenario)) {
            throw new \Mix\Exceptions\InvalidArgumentException("场景未设置");
        }
        $this->_errors      = [];
        ...
        foreach ($scenarioAttributes as $attribute) {
            if (!isset($rules[$attribute])) {
                throw new \Mix\Exceptions\InvalidArgumentException("属性 {$attribute} 未定义规则");
            }
        }
        // 验证器验证
        foreach ($rules as $attribute => $rule) {
            ...
            if (!isset($this->_validators[$validatorType])) {
                throw new \Mix\Exceptions\InvalidArgumentException("属性 {$attribute} 的验证类型 {$validatorType} 不存在");
            }
